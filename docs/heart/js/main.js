...
const canvasBody = mainCanvas.canvas;
const canvas = mainCanvas.context;
$("body").append(mainCanvas.canvas);
...
class Heart {
  constructor(canvas) {
    ...
    const _w = $(window).width() - 200;
    const _h = $(window).height() - 200;

    this.x = _w / 2 + (Math.random() * 20 - Math.random() * 20);
    this.y = _h;
    ...
    this.ID = setInterval(
      function () {
        microparticles.push(
          new MicroParticle(mainCanvas.context, {
            x: this.x,
            y: this.y,
          })
        );
...
class MicroParticle {
  constructor(canvas, options) {
    ...
    this.color = this.random > 0.5 ? "#a9722c" : "#FFFED7";
  }
...
function clear() {
  ...
  canvas.fillRect(0, 0, canvasBody.width, canvasBody.height);
}
