...
Particle = function (p0, p1, p2, p3) {
  ...
  this.w = Particle_Size;
  this.h = Particle_Size;
...
function createParticles() {
  for (var i = 0; i < NUMBER_ITEM; i++) {
    ...
    var p3 = new Point(Math.random() * viewWidth, viewHeight + 50);
...
function animation() {
  ...
  if (phase === 0 && loader.complete) {
    ...
    // End the animation after running once
    console.log("completed");
    window.animationRunning = false;
    return;
  }

  // Continue looping only if not completed
...
function startAnimation() {
  ...
  phase = 0; // Reset to the initial phase
  loader.reset(); // Reset the loader
  exploader.reset(); // Reset the exploader
  particles.length = 0; // Clear any existing particles

  createParticles(); // Recreate particles for the new animation
